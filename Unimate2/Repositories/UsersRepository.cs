...
namespace UniMate2.Repositories
{
    public class UsersRepository : IUsersRepository
    {
        ...
        // Оновлення статусу Shadowban
        public async Task<IdentityResult> ActivateShadowbanAsync(string userId)
        {
            ...
            user.IsShadowbanned = true; // Активуємо Shadowban
            return await _userManager.UpdateAsync(user);
        }

        public async Task<IdentityResult> DeactivateShadowbanAsync(string userId)
        {
            var user = await _userManager.FindByIdAsync(userId);
            if (user == null)
            {
            ...
            user.IsShadowbanned = false; // Деактивуємо Shadowban
            return await _userManager.UpdateAsync(user);
        }

...
namespace UniMate2.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        // POST: Admin/ActivateShadowban
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ActivateShadowban(string userId)
        {
            if (string.IsNullOrEmpty(userId))
            {
                _logger.LogWarning("Shadowban attempt with null or empty ID");
            ...
            // Set the Shadowban status
            user.IsShadowbanned = true; // Activating shadowban
            var updateResult = await _usersRepository.UpdateUserAsync(user);

            if (!updateResult.Succeeded)
            {
                _logger.LogError("Failed to shadowban user: {UserId}", userId);
                TempData["ErrorMessage"] = "Failed to activate Shadowban.";
                return RedirectToAction(nameof(Users));
            }

            _logger.LogInformation("User {UserId} shadowbanned successfully", userId);
            TempData["SuccessMessage"] = "User shadowbanned successfully.";
            return RedirectToAction(nameof(Users));
        }

        // POST: Admin/DeactivateShadowban
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeactivateShadowban(string userId)
        {
            if (string.IsNullOrEmpty(userId))
            {
                _logger.LogWarning("Unshadowban attempt with null or empty ID");
                TempData["ErrorMessage"] = "User ID is required.";
                return RedirectToAction(nameof(Users));
            }
            ...
            // Remove the Shadowban status
            user.IsShadowbanned = false; // Deactivating shadowban
            var updateResult = await _usersRepository.UpdateUserAsync(user);

            if (!updateResult.Succeeded)
            {
                _logger.LogError("Failed to unshadowban user: {UserId}", userId);
                TempData["ErrorMessage"] = "Failed to deactivate Shadowban.";
                return RedirectToAction(nameof(Users));
            }

            _logger.LogInformation("User {UserId} unshadowbanned successfully", userId);
            TempData["SuccessMessage"] = "User unshadowbanned successfully.";
            return RedirectToAction(nameof(Users));
        }
